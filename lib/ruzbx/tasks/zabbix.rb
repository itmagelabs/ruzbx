  ...
  module Tasks
    ...
    class Zabbix
      ...
      def define
        ...
        generate 'attach' do
          parser do
            @parser.banner = "Usage: rake zabbix:attach -- '[options]'"
            @parser.on('-t NAME', '--templates=NAME') { |templates| @options[:templates] = templates.strip.split(',') }
            @parser.on('-h HOST', '--host=HOST') { |host| @options[:host] = host }
          end
          ...
          result = Ruzbx::Api::Host.update(options[:host], { templates: options[:templates] })
